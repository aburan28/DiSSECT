...
def build_features(source, category, bits, traits=None):
    """Join per-trait results for `category` and `category_sim` into one frame."""
    categories = [category, f"{category}_sim"]
...
def usable_features(df, category, max_missing=0.05):
    """Features with real data on both sides, and some variation to speak of."""
    simulated = df[df.category == f"{category}_sim"]
...
def main():
    ...
    for bits in args.bits:
        ...
        df = build_features(args.source, args.category, bits)
        ...
        features = every if args.no_coverage_filter else usable_features(df, args.category, args.max_missing)
